...
class ResultApiTest extends TestCase
{
    ...
    public function test_org_admin_can_submit_and_confirm_result_via_api(): void
    {
        ...
        $this->postJson("/api/v1/events/{$event->id}/matches/{$match->id}/result", [
            ...
            ->assertCreated()
            ->assertJsonPath('data.data.home_score', 1);
        ...
        $this->patchJson("/api/v1/results/{$result->id}/status", [
            ...
            ->assertOk()
            ->assertJsonPath('data.status', ResultStatus::Confirmed->value);
    }
}