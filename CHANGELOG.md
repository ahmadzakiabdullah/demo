...
- POLISH-06: Eligibility rules engine completed with full weight category support (migration, model, service checks, form inputs in Create/Edit, enforcement in registration workflow and status transitions, tests). All POLISH items from Phase 1-3 backlog now addressed (no pending remaining).
- POLISH-07: Basic venue/facility availability (upcoming bookings list/calendar in Venues/Show) + blocking (existing conflict detector).
- POLISH-03: Full event_participant_id propagation to medals and rankings tables (new migration + columns). Updated allocators/calculators to populate it from teams/athletes. Enhanced medal tally with by_contingent view in UI. Backfill happens automatically on result confirmation / draw generation.
